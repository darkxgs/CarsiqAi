...
export function getVerifiedOilFilter(make: string, model: string, year?: number): FilterRecommendation | null {
  try {
    ...
    if (!make || !model || typeof make !== 'string' || typeof model !== 'string') {
      logger.warn('Invalid parameters provided to getVerifiedOilFilter', { make, model, year });
      return null;
    }
...
export function getVerifiedAirFilter(make: string, model: string, year?: number): FilterRecommendation | null {
  try {
    // Validate input parameters
    if (!make || !model || typeof make !== 'string' || typeof model !== 'string') {
      logger.warn('Invalid parameters provided to getVerifiedAirFilter', { make, model, year });
      return null;
    }
    ...
    const relevantResults = searchResults.filter(result => 
      result.brands && make && typeof make === 'string' &&
      result.brands.some(brand => brand && typeof brand === 'string' && brand.toLowerCase() === make.toLowerCase())
    );
...
export function searchFiltersWithArabicSupport(query: string): Array<{
  ...
  if (!query || typeof query !== 'string') {
    console.warn('Invalid query provided to searchFiltersWithArabicSupport', { query });
    return results;
  }
