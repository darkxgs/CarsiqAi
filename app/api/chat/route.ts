...
⚠️ **قاعدة مهمة جداً:** إذا تم توفير قائمة "منتجات الزيوت المتوفرة في المتجر" في السياق، يجب استخدام هذه المنتجات فقط في التوصيات!

🥇 **الخيار الأول :** [Brand Name] [Product Line] [Viscosity] ([السعر دينار]) - [الحالة]
🥈 **الخيار الثاني :** [Brand Name] [Product Line] [Viscosity] ([السعر دينار]) - [الحالة]
🥉 **الخيار الثالث :** [Brand Name] [Product Line] [Viscosity] ([السعر دينار]) - [الحالة]
📦 **فلتر الزيت:** [رقم Denckermann]
🌬️ **فلتر الهواء:** [رقم Denckermann]
❄️ **فلتر المكيف:** [رقم Denckermann]

**مثال على التنسيق المطلوب (مع منتجات متوفرة):**
🥇 **الخيار الأول :** Valvoline Advanced 0W-20 (25000 دينار) - متوفر: 10 قطع
🥈 **الخيار الثاني :** Castrol EDGE 0W-20 (28000 دينار) - متوفر: 8 قطع
🥉 **الخيار الثالث :** Liqui Moly Top Tec 6600 0W-20 (32000 دينار) - متوفر: 5 قطع
...
**مثال على التنسيق (بدون منتجات متوفرة):**
...
📱 **زر الواتساب للطلب (إجباري - لا تنساه أبداً!):**

⚠️ **قاعدة إلزامية:** في نهاية كل رد يحتوي على توصيات زيوت، يجب إضافة هذا القسم بالضبط:
...
🚨 **تحذير:** عدم إضافة زر الواتساب = خطأ فادح! يجب إضافته في كل رد يحتوي على توصيات منتجات.
...
1. ✅ استخدم المنتجات المتوفرة في المتجر (إذا تم توفيرها في السياق)
2. ✅ اذكر الأسعار والكميات المتوفرة
3. ✅ أضف زر الواتساب في نهاية كل رد
4. ✅ استخدم التنسيق الصحيح مع الإيموجي
5. ✅ أضف خط فاصل (---) قبل وبعد قسم الواتساب
` ,
  headers: {
      ...
      if (viscosityMatch) {
        ...
        if (products && products.length > 0) {
          const productLines: string[] = [
            '\n\n🛒 منتجات الزيوت المتوفرة في المتجر (استخدم هذه المنتجات في توصياتك):',
            ''
          ]

          products.forEach((product: any, index: number) => {
            const stockInfo = product.stock > 0 ? `متوفر (${product.stock} قطعة)` : 'غير متوفر حالياً'
          ...
          productLines.push(
            '⚠️ تعليمات مهمة للذكاء الاصطناعي:',
            '• استخدم المنتجات المتوفرة أعلاه في توصياتك (الخيار الأول، الثاني، الثالث)',
            '• اذكر السعر والتوفر لكل منتج',
            hasExactMatch ? 
              '• المنتجات أعلاه تطابق اللزوجة المطلوبة تماماً' :
              `• ملاحظة: اللزوجة المطلوبة ${viscosity} غير متوفرة، المنتجات أعلاه بدائل مناسبة - اذكر ذلك للعميل`,
            '• إذا كان المنتج غير متوفر، اذكر ذلك بوضوح',
            '• رتب المنتجات حسب الأولوية: المتوفر أولاً، ثم حسب السعر',
            ''
          )
