  ...
  const renderedContent = useMemo(() => {
    ...
    const parseHtml = (htmlContent: string) => {
      ...
      return paragraphs.map((paragraph, i) => {
        ...
        // Process paragraph content to restore bold tags and handle links
        const processParagraphContent = (text: string) => {
          // First, handle markdown links [text](url)
          const linkRegex = /\[([^\]]+)\]\(([^)]+)\)/g
          const textWithLinks = text.replace(linkRegex, '___LINK_START___$1___LINK_URL___$2___LINK_END___')
          
          const parts = textWithLinks.split(/(___BOLD_START___|___BOLD_END___|___LINK_START___|___LINK_URL___|___LINK_END___)/g)
              ...
              if (linkText && linkUrl) {
                result.push(
                  <a 
                    ...
                    className="inline-flex items-center gap-1 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors duration-200 no-underline"
                  >
                    {linkText}
                  </a>
                )
